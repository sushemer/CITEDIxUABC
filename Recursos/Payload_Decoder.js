	...
	decoder = [
		{
			key: [0x00, 0xFF],
			fn: function(arg) { 
				decoded_data['battery_voltage'] = (decode_field(arg, 2, 15, 0, "signed") * 0.01).toFixed(3);
				return 2;
			}
		},
		{
			...
			fn: function(arg) { 
				...
				{switch (val){
					case 0:
						decoded_data['hall_effect_state'] = "Magnet_Present";
						break;
					case 255:
						decoded_data['hall_effect_state'] = "Magnet_Absent";
						break;
					default:
		...
		},
		{
			key: [0x0F, 0x04],
			fn: function(arg) { 
				decoded_data['EXTERNAL_CONNECTOR_COUNT'] = decode_field(arg, 2, 15, 0, "unsigned");
				return 2;
			}
		},
		{
			key: [0x12, 0x04],
			fn: function(arg) { 
				decoded_data['EXTERNAL_CONNECTOR_TOTAL_COUNT'] = decode_field(arg, 4, 31, 0, "unsigned");
				return 4;
			}
		},
		{
			key: [0x11, 0x02],
			fn: function(arg) { 
				decoded_data['EXTERNAL_CONNECTOR_ANALOG'] = (decode_field(arg, 2, 15, 0, "signed") * 0.001).toFixed(3);
				return 2;
			}
		},
		{
		...
		},
		{
			key: [0x04, 0x68],
			fn: function(arg) { 
				decoded_data['HUMIDITY'] = (decode_field(arg, 1, 7, 0, "unsigned") * 0.5).toFixed(1);
				return 1;
			}
		},
		{
...
if (port === 5) {
    ...
    try {
        ...
        decoded.data.LORA_RSSI = rawPayload.payloadMetaData.gatewayMetaDataList[0].rxInfo.rssi;
        decoded.data.LORA_SNR = rawPayload.payloadMetaData.gatewayMetaDataList[0].rxInfo.loRaSNR;
